...
def analyse_results(dump_folder, datafile, data, analyse_func, headers_func):
    ...
    chars, speakers, utts = get_characteristics(datafile)
...
def get_characteristics(datafile, speakers=None):
    """
    ...
    utts = list()  # stores the utterance IDs
    if speakers is None:  # `speakers` is a list of unique speaker IDs
        speakers = list()
    elif isinstance(speakers, np.ndarray):
    ...
    return chars, speakers_idx, utts
